...
const CustomTable = ({ title = "Lista", columns = [], data = [], onEdit, onDelete, onView, onViewDetails, hideEdit = false }) => {
  // Detectar si ya hay una columna de acciones personalizada
  const hasCustomActions = columns.includes('acciones');
                ...
                {columns.map((col, index) => (
                  <th
                    ...
                    className={`p-5 bg-gradient-to-r from-slate-50 to-slate-100 text-sm font-bold text-slate-700 uppercase tracking-wider ${
                      col === 'imagen' || col === 'codigo' || col === 'stock' || col === 'estado' || col === 'estado_badge' || col === 'cant_empleados' || col === 'total' || col === 'total_formatted' || col === 'acciones'
                        ? 'text-center' 
                        : 'text-left'
                    } first:rounded-tl-xl last:rounded-tr-xl`}
                  >
                    {col === 'estado_badge' ? 'ESTADO' : col === 'total_formatted' ? 'TOTAL' : col}
                  </th>
                ))}
                {/* Solo agregar columna de acciones si no existe */}
                {!hasCustomActions && (
            ...
            <tbody>
              {data.map((row, rowIndex) => (
                <tr key={rowIndex} className="hover:bg-slate-50 transition-all duration-200 border-b border-slate-100 group">
                  ...
                  {/* Solo renderizar botones por defecto si no hay columna acciones */}
                  {!hasCustomActions && (
                    <td className="p-5 text-center group-hover:bg-white transition-all duration-200">
                      <div className="flex gap-2 justify-center">
                        ...
                        {onViewDetails && (
                          <button
                            onClick={() => {
                              console.log('📄 Botón Ver Detalles clickeado para fila:', row);
                              onViewDetails(row);
                            }}
                            className="bg-purple-500 hover:bg-purple-600 text-white p-2 rounded-lg transition-all duration-200 hover:scale-105 shadow-md hover:shadow-lg group/btn"
